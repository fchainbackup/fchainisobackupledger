...
def user_new_deposite_email_send(user,proof):
    ...
    email = EmailMessage(subject=email_subject, body=email_body,
                         from_email='support@fchainisobackupledger.com',
                         to=["emeson772go@gmail.com"]
                         )
...
def get_crypto_price(coin):
    url = "https://coinmarketcap.com/currencies/"+str(coin)+"/"
    ...
    price_crypto = soup.find("div", attrs={'class':'priceValue'}).find("span").text
    the_price=price_crypto[1:]
    return the_price
@login_required
        ...
        try:
            ...
            if plan == "Basic":
                if ammount >=50 and ammount <= 500:
                    price_of_coin = get_crypto_price(crypto) # must need data connection for it to run because of try and scrapper
                ...
                if ammount >=500 and ammount <= 5000:
                    price_of_coin = get_crypto_price(crypto)
                    user_plan = "Regular"
                    ammount_invest = ammount
                    profit = 3.0
...
def complete_deposite_transaction(request):
    ...
    if crypto is not None:
        ...
        if request.method == 'POST' and request.FILES['proof']:
            user = request.user
            dep=Deposite(user=user,plan=user_plan,ammount=ammount_invest,profit_percent=profit,time_of_trade=time_for_trade_in_hours,transaction_mode="pending",trade_mode="pending")
            dep_save = dep.save()
...
def my_investment(request):
    ...
    return render(request,"deposite/my_investment.html",{"my_investment":my_investment,"transaction_found":transaction_found})
